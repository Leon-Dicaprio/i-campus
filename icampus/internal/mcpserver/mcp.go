...
type ClientManager struct {
	...
	ToolMap map[string]*MCPHTTPClient
}
...
func NewClientManager(configFile string) (*ClientManager, error) {
	...
	mgr := &ClientManager{
		...
		ToolMap: make(map[string]*MCPHTTPClient),
	}
...
func (m *ClientManager) LoadTools() ([]openai.Tool, error) {
	var allTools []openai.Tool
	for _, client := range m.Clients {
		// 标准 MCP 协议：初始化后调用 tools/list
		// 这里假设服务端不需要复杂的 initialize 参数，直接调 tools/list
		// 严谨的做法应该先发 initialize 消息
...
func NewMCPAgent(configFile, apiKey string) (*MCPAgent, error) {
	...
	// 2. 获取工具
	tools, err := mgr.LoadTools()
	if err != nil {
		log.Printf("警告: 工具加载不完全: %v", err)
	}
	fmt.Printf("成功加载 %d 个工具\n", len(tools))
