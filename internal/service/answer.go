...
func (s *Service) Answer(ctx context.Context, messages []history.Message) (string, error) {
	if len(messages) == 0 {
		return "", fmt.Errorf("no messages provided")
	}
	question := messages[len(messages)-1].Content

	decision, err := s.DecisionAgent.Decide(ctx, messages)
	if err != nil {
	...
	prompt := buildFinalPrompt(ctxText, messages)
...
func (s *Service) AnswerStream(ctx context.Context, messages []history.Message) (<-chan StreamEvent, error) {
	...
	go func() {
		...
		if len(messages) == 0 {
			ch <- StreamEvent{Type: "error", Content: "no messages provided"}
			return
		}
		question := messages[len(messages)-1].Content
...
func buildFinalPrompt(contextText string, messages []history.Message) string {
	historyText := formatConversationHistory(messages)
...
学生问题：%s`, contextText, historyText, question)
}
...
func formatConversationHistory(messages []history.Message) string {
...
func lastUserQuestion(messages []history.Message) string {
